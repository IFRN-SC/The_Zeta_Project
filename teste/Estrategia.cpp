...
void Estrategia::contornarObstaculo(){
   ...
   delay(1000);     
   robo.acionarMotores(-25, -25);//da rer um pouco
   delay(500);
   robo.acionarMotores(0, 0);
   delay(1000);  
   robo.acionarMotores(-30,30);
...
void Estrategia::subir_rampa(){
  ...
  while(((valor_sensor_sonar_lateral_Esquerdo > 1 && valor_sensor_sonar_lateral_Esquerdo < 10 && valor_sensor_sonar_lateral_Direito > 1 && valor_sensor_sonar_lateral_Direito <  10))){
    robo.ligarLed(led3);
